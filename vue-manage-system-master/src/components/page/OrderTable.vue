    ...
    <div>
        ...
        <div class="container">
            <div class="handle-box">
                ...
                <el-button type="primary"  @click="cancleSearch">取消搜索</el-button>
            </div>
            <el-table
                    :data="tableData"
                    border
            ...
            >
                ...
                <el-table-column prop="orderid" label="订单ID" width="150" align="center"></el-table-column>
                <el-table-column prop="allnum" label="订单数量"></el-table-column>
                <el-table-column prop="allmoney" label="订单金额"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 获取 easy-mock 的模拟数据
            getOrderDate() {
                axios.get('http://localhost:8181/order/findAllPage/0/7',).then(resp=>{
                    this.tableData=resp.data.content;
                    this.pageTotal=resp.data.totalElements;
                    Object.values(resp.data.content).forEach((value,index)=>{
                      let timesss= Date.parse(value.createtime);
                      this.tableData[index].createtime=new Date(parseInt(timesss)).toLocaleString()
                    });
            ...
            handlePageChange(currentPage) {
                ...
                axios.get('http://localhost:8181/order/findAllPage/'+pageNow+'/7').then(resp=>{
                    this.tableData=resp.data.content;
                    this.pageTotal=resp.data.totalElements;
                    Object.values(resp.data.content).forEach((value,index)=>{
                        let timesss= Date.parse(value.createtime);
            ...
            handleSearch() {
             ...
             /*   axios.get('http://localhost:8181/order/findByDate').then(resp=>{
                    console.log(resp);
                });*/
                axios({
                    methods: "GET",
                    url:'http://localhost:8181/order/findByDate',
            ...
            handleDelete(index, row) {
                ...
                this.$confirm('确定要删除吗？', '提示', {
                    ...
                    .then(() => {
                        ...
                        axios.delete('http://localhost:8181/order/deleteByOrderId',{params:{
                            orderid: row.orderid
                            }}).then(resp=>{
                            console.log(resp);
                        })
            ...
            handleEdit(index, row) {
                ...
                console.log(row);
                console.log(row.orderid);
                axios.get('http://localhost:8181/orderDetail/findODById',row.orderid).then(resp=>{
                    console.log(resp);
